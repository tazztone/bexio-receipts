...
def test_healthz_ok(test_settings):
    ...
    with patch("sqlite3.connect") as mock_sql:
        mock_sql.return_value.__enter__.return_value.execute.return_value.fetchone.return_value = (1,)
...
def test_setup_checks(test_settings):
    ...
    with patch("bexio_receipts.server.BexioClient") as mock_bexio:
        ...
        mock_inst.client.get.return_value.json = MagicMock(return_value={"name": "Test Co"})
...
def test_pull_model_success(test_settings):
    ...
    with patch("httpx.AsyncClient.post", new_callable=AsyncMock, return_value=mock_resp):
        response = client.post("/setup/pull-model", data={"model": "test-model"}, auth=("admin", "test_password"))
...
def test_discard_review(test_settings, tmp_path):
    ...
    with patch("fastapi.Request.session", new_callable=MagicMock) as mock_session:
        ...
        response = client.post("/discard/test", data={"csrf_token": "test_token"}, auth=("admin", "test_password"), follow_redirects=False)
    ...
    with patch("starlette.middleware.sessions.SessionMiddleware.__call__") as mock_session:
...
def test_push_to_bexio_success(test_settings, tmp_path):
    ...
    with patch("bexio_receipts.server.BexioClient") as mock_bexio:
        ...
        with patch("bexio_receipts.server.Request.session", property(lambda x: {"csrf_token": "test_token"})):
            response = client.post("/push/test_id", data=form_data, auth=("admin", "test_password"), follow_redirects=False)
            assert response.status_code == 303
            assert not review_file.exists()
