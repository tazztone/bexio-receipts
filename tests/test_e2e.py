...
async def test_pipeline_e2e(tmp_path):
    ...
    respx.get("https://api.bexio.com/3.0/profile/me").mock(return_value=httpx.Response(200, json={"id": 1, "name": "Test"}))
    respx.get("https://api.bexio.com/3.0/taxes").mock(return_value=httpx.Response(200, json=[{"id": 1, "value": 8.1}]))
    respx.get("https://api.bexio.com/2.0/accounts").mock(return_value=httpx.Response(200, json=[{"id": 100, "account_no": "6000"}]))

    respx.post("https://api.bexio.com/3.0/files").mock(return_value=httpx.Response(200, json={"id": "file-123", "name": "dummy.png"}))

    respx.post("https://api.bexio.com/2.0/contact/search").mock(return_value=httpx.Response(200, json=[]))
