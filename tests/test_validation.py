...
def test_valid_receipt(test_settings):
    ...
    errors, warnings = validate_receipt(receipt, test_settings)
    assert not errors
    assert not warnings
...
def test_rounding_tolerance(test_settings):
    ...
    errors, _ = validate_receipt(receipt, test_settings)
    assert not errors
...
def test_old_date(test_settings):
    ...
    errors, warnings = validate_receipt(receipt, test_settings)
    assert any("Receipt older than 365 days" in w for w in warnings)
    assert not errors
