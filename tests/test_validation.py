...
def test_vat_breakdown_cross_check(test_settings):
    receipt = Receipt(
        ...
        total_incl_vat=10.81,
        vat_breakdown=[
            VatEntry(rate=8.1, base_amount=15.0, vat_amount=1.21) # Base is 15.0 != subtotal 10.0
        ]
    )
    errors = validate_receipt(receipt, test_settings)
    assert any("VAT breakdown base (15.0) ≠ subtotal (10.0)" in e for e in errors)
    assert any("VAT breakdown total (1.21) ≠ extracted VAT amount (0.81)" in e for e in errors)
    assert any("VAT breakdown sum (16.21) ≠ total incl. VAT (10.81)" in e for e in errors)
...
def test_line_items_mismatch_no_subtotal(test_settings):
    receipt = Receipt(
        ...
        line_items=[
            LineItem(description="Item 1", unit_price=10.0, total=10.0),
            LineItem(description="Item 2", unit_price=11.0, total=11.0) # Sum = 21.0 != total 20.0
        ]
    )
    errors = validate_receipt(receipt, test_settings)
